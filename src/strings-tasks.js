 ...
 * @return {number} - The first character of the string.
 *
 * @example
...
function isPalindrome(str) {
  const filter = str.replace(/[\s!?,]/g, '').toLowerCase();
  let remove;

  if (filter.length % 2 === 0) {
    remove = filter;
  } else {
    const middleIndex = Math.floor(filter.length / 2);
    remove = filter.slice(0, middleIndex) + filter.slice(middleIndex + 1);
  }

  const v1 = remove.slice(0, Math.floor(remove.length));
  const v2 = remove
    .split('')
    .reverse()
    .join('')
    .slice(0, Math.floor(remove.length));
  return v1 === v2;
}
...
function reverseWords(str) {
  return str
    .split(' ')
    .reduce((acc, curr) => [...acc, curr.split('').reverse().join('')], [])
    .join(' ');
}
...
function invertCase(str) {
  const transformedArray = str.split(' ').map((sElement) =>
    sElement
      .split('')
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (str[i].toUpperCase() !== str[i].toLowerCase()) {
      res.push(output[input.indexOf(str[i])]);
    } else {
      res.push(str[i]);
    }
  }
